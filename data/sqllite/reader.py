...
from yearbook.Yearbook import create_yearbook_from_db, Yearbook, create_yearbook
...
def get_tree_model(dir_params: {}, school_selection: str):
    ...
    for row in all_rows:
        ...
        if current_grade not in added_schools[school_name].keys():
            ...
            [grade_page.parent_pages.append(school_page) for grade_page, school_page in zip(grade_yearbook.pages, school_yearbook.pages)]
        ...
        if current_class not in added_schools[school_name][current_grade].keys():
            class_yearbook = create_yearbook(dir_params, school_name, grade=current_grade, classroom=current_class, child=None)

            # Set the parent pages for this yearbook
            [class_page.parent_pages.append(grade_page) for class_page, grade_page in zip(class_yearbook.pages, grade_yearbook.pages)]
        ...
        if current_child not in added_schools[school_name][current_grade][current_class].keys():
            child_yearbook = create_yearbook(dir_params, school_name, grade=current_grade, classroom=current_class, child=current_child)
            treestore.append(class_parent, [child_yearbook])
            added_schools[school_name][current_grade][current_class][current_child] = {}

            # Set the parent pages for this yearbook
            [child_page.parent_pages.append(class_page) for child_page, class_page in zip(child_yearbook.pages, class_yearbook.pages)]
